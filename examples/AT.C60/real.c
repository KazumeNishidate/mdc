...
void   check_vdw(void){
  ...
  for(i=0;i<1000;i++){
    ...
    printf("%lf %lf  %lf  %lf  %lf\n",rij, rij/vdw.sig, pot/vdw.eps,fxi*(vdw.sig/vdw.eps),fxj*(vdw.sig/vdw.eps));
...
void   calc_vdw(void){
  ...
  if(cnt==0){
    ...
    rdy = sys.Ly*0.5;
    rdz = sys.Lz*0.5;
    if(vdw.cut_out > rdx) {   // to prevent the double count in the vdW calc.
      printf("caution! \n");    
      printf("sys.Lx is smaller than the half of the vdw.cut_out.\n");
      printf("vdw.cut_out is changed from %lf to %lf to avoid the inconsistency\n\n",vdw.cut_out, rdx);
      vdw.cut_out = rdx;
    }
    if(vdw.cut_out > rdy) {
      printf("caution! \n");      
      printf("sys.Ly is smaller than the half of the vdw.cut_out.\n");    
      printf("vdw.cut_out is changed from %lf to %lf to avoid the inconsistency\n\n",vdw.cut_out, rdy);
      vdw.cut_out = rdy;
    }
    if(vdw.cut_out > rdz) {
      printf("caution! \n");
      printf("sys.Lz is smaller than the half of the vdw.cut_out.\n");        
      printf("vdw.cut_out is changed from %lf to %lf to avoid the inconsistency\n\n",vdw.cut_out, rdz);
      vdw.cut_out = rdz;
    }
  }
  ...
  for(i=0;i<sys.N;i++) {
    for(j=i+1;j<sys.N;j++) {
      ...
      if(rij > vdw.cut_out) continue; // F_vdW active region {r}:  cut_in < r < cut_out
      if(rij < vdw.cut_in ) continue;  
...
void	real_space(void) {
  ...
  // enable the following line to include the vdW force
  calc_vdw();      
